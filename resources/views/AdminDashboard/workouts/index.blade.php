    ...
    <div class="flex-page d-flex row wrap">
     ...
     <div class="card-header pb-0">
        <div class="col-sm-1 col-md-2">
                <a class="btn btn-primary btn-sm" href="{{ route('workouts.create') }}"> Add New workout</a>
        </div>
    </div>
<br><br>
            </div>
<table>
    ...
    <tr>
      ...
      <td>
        <a href="{{ url('workouts/' . $item->id ) }}">
          <button class="btn btn-info mb-2" > view </button>
        </a>
      </td>

      <td>
        <a href="{{ url('workouts/' . $item->id) . '/edit'}}">
            <button > Update </button>
        </a>
      </td>

      <td>
        <form method="POST" action="{{ url('/workouts'. '/' . $item->id) }}"  >
          {{ method_field('DELETE') }}
          {{ csrf_field() }}
