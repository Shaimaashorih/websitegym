...
function displayRow($id, $timetables) {
    for ($x = 0; $x < count($timetables); $x++) {
        echo "<tr>";        
        if ($x == 0) {
            echo "<td rowspan=\"" . count($timetables) . "\">" . $id . "</td>";
        }
        echo "<td>" . $timetables[$x] . "</td>";
        echo "</tr>";
    }
}
...
while ($sentencie->fetch()) {
    if ($id != $id_prev) {
        displayRow($id, $timetables);
        $products = array();
        $id_prev = $id;
    }
    array_push($timetables, $timetables);
}
...
{{-- <table>
    ...
    <tr>
      ...
      <td>
        <a href="{{ url('timetables/' . $item->id ) }}">
          <button class="btn btn-info mb-2" > view </button>
        </a>
      </td>

      <td>
        <a href="{{ url('timetables/' . $item->id) . '/edit'}}">
            <button > Update </button>
        </a>
      </td>

      <td>
        <form method="POST" action="{{ url('/timetables'. '/' . $item->id) }}"  >
          {{ method_field('DELETE') }}
          {{ csrf_field() }}
