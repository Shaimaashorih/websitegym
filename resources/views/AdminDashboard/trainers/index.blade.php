    ...
    <div class="flex-page d-flex row wrap">
     ...
     <div class="card-header pb-0">
        <div class="col-sm-1 col-md-2">
                <a class="btn btn-primary btn-sm" href="{{ route('trainers.create') }}"> Add New Trainer</a>
        </div>
    </div>
<br><br>
...
<table>
    ...
    <tr>
      ...
      <td>{{$item->email}}</td>
      <td>
        <img src="{{ asset('images/'.$item->image)}}" style="width: 70px; height:70px;">
      </td>

      <td>
        <a href="{{ url('trainers/' . $item->id ) }}">
          <button class="btn btn-info mb-2" > view </button>
        </a>
      </td>

      <td>
        <a href="{{ url('trainers/' . $item->id) . '/edit'}}">
            <button > Update </button>
        </a>
      </td>

      <td>
        <form method="POST" action="{{ url('/trainers'. '/' . $item->id) }}"  >
          {{ method_field('DELETE') }}
          {{ csrf_field() }}
